...
import PushToShiprocket from "@/components/admin/PushToShiprocket"; 
...
export default async function AdminOrderDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const isOrderShipped = order.status === "SHIPPED" || order.status === "DELIVERED";
      ...
      <div className="bg-white p-4 sm:p-8 rounded-3xl shadow-sm border border-slate-100">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 border-b border-slate-100 pb-6 gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
             ...
             {/* 💡 PRO FEATURE: Passing isShipped Prop */}
             <PushToShiprocket orderId={order.id} isShipped={isOrderShipped} />
          </div>
        </div>
